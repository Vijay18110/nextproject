    ...
    <ul className="d-none d-md-flex list-unstyled mb-0 gap-4 align-items-center">
      ...
      <li key={idx} className="position-relative">
        ...
        <div className="d-flex align-items-center gap-1 cursor-pointer text-dark fw-medium" onMouseEnter={()=>
          ...
          >
          <Link href={link?.submenu ? "#" : link.href} className={`px-3 py-2 rounded-3 text-decoration-none ${
            pathname===link.href ? "active-gradient activeTextColor" : "text-dark hover-bg" }`}>
          {link.name}
          </Link>
        ...
        <Link href={link.href} className={`d-flex align-items-center gap-2 px-3 py-2 rounded-3 text-decoration-none
          fw-medium ${ pathname===link.href ? "active-gradient activeTextColor" : "text-dark hover-bg" }`}>
        {link.icon}
        {link.name}
        </Link>
        )}
  ...
  <AnimatePresence>
    ...
    <motion.div initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }}
      ...
      <div key={idx}>
        ...
        <AnimatePresence>
          {activeSubMenu === link.name && (
          <motion.ul initial={{ height: 0, opacity: 0 }} animate={{ height: "auto" , opacity: 1 }} exit={{ height: 0,
            opacity: 0 }} transition={{ duration: 0.3 }} className="list-unstyled ms-4 mb-3">
            {link.submenu.map((sub) => (
            <motion.li key={sub.href} whileHover={{ x: 5 }}>
              <Link href={sub.href} onClick={()=> setIsOpen(false)}
              className="d-flex align-items-center gap-2 px-2 py-2 text-decoration-none text-dark rounded-2
              submenu-hover"
              >
              {sub.icon && (
              <span className="text-primary">{sub.icon}</span>
