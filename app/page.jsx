...
const PRODUCT_DATA_3 = [...PRODUCT_DATA_1, ...PRODUCT_DATA_1];
      ...
      {data.map((item, idx) => (
        <motion.div key={`${item.id}-${idx}`} className={isSmall ? "col-md-3 col-sm-6" : "col-md-4 col-sm-6"} variants={listItem}>
          <div className={`${isSmall ? 'product-card1' : 'product-card'} shadow-sm rounded overflow-hidden position-relative`}>
            <motion.div variants={imageVariant} style={{ overflow: "hidden" }}>
              <Image src={item.image} alt={item.title} width={500} height={isSmall ? 400 : 400} className="w-100 product-img" />
            </motion.div>
...
const ProductCardItem = ({ item }) => (
  <motion.div className="col-md-4 col-sm-6" variants={listItem}>
    <div className="product-card shadow-sm rounded overflow-hidden position-relative">
      <motion.div variants={imageVariant} className="w-100" style={{ overflow: "hidden" }}>
        <Image src={item.image} alt={item.title} width={550} height={600} className="w-100 product-img" />
      </motion.div>
      <div className="product-overlay">
        <h4 className="text-white fw-semibold">{item.title}</h4>
      </div>
