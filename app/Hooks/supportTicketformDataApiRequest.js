...
const useSupportFormDataApiRequest = () => {
  ...
  const sendApiRequest = async (
    ...
    try {
      ...
      let res;
      if (method === "POST") {
        res = await axiosInstancesupport.post(url, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      } else if (method === "GET") {
        const params = new URLSearchParams(data).toString();
        res = await axiosInstancesupport.get(`${url}?${params}`);
      } else {
...
export default useSupportFormDataApiRequest;
